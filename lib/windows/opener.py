    ...
    if isinstance(obj, playqueue.PlayQueue):
        if busy.widthDialog(obj.waitForInitialization, None):
            if obj.type == "audio":
                ...
                return handleOpen(
                    musicplayer.MusicPlayerWindow, track=obj.current(), playlist=obj
                )
            elif obj.type == "photo":
                from . import photos
...
def genreClicked(genre, **kwargs):
    ...
    filter_ = {
        "type": genre.FILTER,
        "display": "Genre",
        "sub": {"val": genre.id, "display": genre.tag},
    }
    return sectionClicked(section, filter_, **kwargs)


def directorClicked(director, **kwargs):
    from . import director as director_window

    return handleOpen(director_window.DirectorWindow, role=director, **kwargs)
